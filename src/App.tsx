...
export default function App() {
  ...
  const clearDevelopmentDemoTimers = () => {
    devDemoTimers.current.forEach((timer) => window.clearTimeout(timer));
    devDemoTimers.current = [];
  };
  ...
  const runAiDevelopmentDemo = () => {
    ...
    setBranch("main");
    setGraph(true);
    save(title, [], null, "main", null, newId);

    demoCommits.forEach((commit, index) => {
      const timer = window.setTimeout(() => {
        const partial = demoCommits.slice(0, index + 1);
        setCommits(partial);
        cRef.current = partial;
        setHeadId(commit.id);
        setBranch(commit.branch);
        setScrollTarget(commit.id);
        save(title, partial, commit.id, index === demoCommits.length - 1 ? "main" : commit.branch, null, newId);

        if (index === demoCommits.length - 1) {
          setBranch("main");
          setDevDemoRunning(false);
          devDemoTimers.current = [];
        }
