...
import { commonEventColor, commonEventLabel } from "@/lib/common-events";
import { developmentEventColor, developmentEventLabel } from "@/lib/development-events";
import {
  ResizablePanelGroup,
  ResizablePanel,
  ResizableHandle,
      ...
      <div className="flex h-12 shrink-0 items-center justify-between gap-2 px-3">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <div className="flex rounded-md border bg-background p-0.5">
            <Button
              type="button"
              variant={developmentMode === "demo" ? "secondary" : "ghost"}
              size="sm"
            ...
            <Button
              type="button"
              variant={developmentMode === "live" ? "secondary" : "ghost"}
              size="sm"
          ...
          {developmentMode !== "live" && runAiDevelopmentDemo && (
            <Button
              ...
              variant="outline"
              size="sm"
              onClick={runAiDevelopmentDemo}
              disabled={devDemoRunning}
              className="h-8 max-w-[240px] gap-2 px-3 text-xs"
            >
              <GitBranch className="size-3.5 shrink-0" />
              <span className="truncate">
                {devDemoRunning ? "Running AI Development Demo" : "Run AI Development Demo"}
              </span>
            </Button>
          )}
            ...
            <div className="flex flex-col items-center gap-8">
              ...
              {developmentMode !== "live" && runAiDevelopmentDemo && (
                <button
                  ...
                  onClick={runAiDevelopmentDemo}
                  disabled={devDemoRunning}
                  className="group w-full max-w-[760px] rounded-lg border border-dashed border-[color:var(--branch-1)]/60 bg-card px-5 py-4 text-left transition-all hover:border-[color:var(--branch-1)] hover:shadow-md disabled:cursor-not-allowed disabled:opacity-60"
                >
                  <div className="flex items-center gap-2 mb-1">
                    <GitBranch className="size-4 text-[color:var(--branch-1)]" />
            ...
            <div className="mx-auto flex w-full max-w-[760px] flex-1 flex-col gap-8 px-6">
              ...
              >
                ...
                <div className="self-start w-full flex flex-col gap-1">
                  {(cm.liveEvent || cm.developmentEvent) && (() => {
                    const eventColor = commonEventColor(cm.liveEvent) || developmentEventColor(cm.developmentEvent);
                    return (
                      <div
                        className="mb-1 inline-flex w-fit items-center gap-2 rounded-md border px-2 py-1 text-[11px] font-semibold"
                        style={{
                          borderColor: eventColor || undefined,
                          color: eventColor || undefined,
                          backgroundColor: eventColor ? eventColor + "1A" : undefined,
                        }}
                      >
                        <span>{cm.liveEvent ? commonEventLabel(cm.liveEvent) : developmentEventLabel(cm.developmentEvent)}</span>
                      </div>
                    );
                  })()}
                  {cm.role && (
