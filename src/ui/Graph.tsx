...
export default function Graph(props: Props) {
  ...
  const commitMap = new Map<string, any>(commits.map((c: any) => [c.id, c]));
  ...
  const nodeColor = (node: any) => {
    const cm = node?.cid ? commitMap.get(node.cid) : null;
    return commonEventColor(cm?.liveEvent) || developmentEventColor(cm?.developmentEvent) || bCol(names, node.branch);
  };
      ...
      <svg width={W} height={H} style={{ display: "block" }}>
        ...
        {vnodes.map((n) => {
          ...
          return parents.map((pid: string) => {
            ...
            const col = isGhostEdge ? mutedFg : nodeColor(n);
        ...
        {vnodes.map((n) => {
          ...
          const cm = commitMap.get(n.cid);
          const col = commonEventColor(cm?.liveEvent) || developmentEventColor(cm?.developmentEvent) || roleColor(cm?.role) || bCol(names, n.branch);
          ...
          const displayText = cm?.liveEvent?.label || cm?.developmentEvent?.title || cm?.displayLabel || (cm?.prompt || "").replace(/\s+/g, " ").trim();
          ...
          return (
            ...
            >
              ...
              {hoverNodeId === n.cid && (cm?.liveEvent || cm?.developmentEvent || cm?.model) && (
                <text x={lX} y={p.y + 17} fontSize={9} fill={mutedFg} style={{ fontFamily: "system-ui", pointerEvents: "none" }}>
                  {cm?.liveEvent ? commonEventLabel(cm.liveEvent) : cm?.developmentEvent ? developmentEventLabel(cm.developmentEvent) : shortModelName(cm.model)}
                </text>
              )}
            </g>
          );
