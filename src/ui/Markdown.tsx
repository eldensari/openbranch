...
export function renderMd(text: string, chips?: CitationLike[]): React.ReactNode[] | null {
  ...
  while (i < lines.length) {
      ...
      elements.push(
        ...
        >
          {quoteLines.map((q, idx) => (
            <div key={idx} className="text-[17px] leading-relaxed">
    ...
    if (line.startsWith("### ")) {
      elements.push(
        <h3 key={kr.k++} className="mt-4 mb-1 text-[17px] font-semibold">
    ...
    if (line.startsWith("## ")) {
      elements.push(
        <h2 key={kr.k++} className="mt-4 mb-1 text-[19px] font-semibold">
    ...
    if (line.startsWith("# ")) {
      elements.push(
        <h1 key={kr.k++} className="mt-4 mb-1 text-[22px] font-semibold">
    ...
    if (/^[-*] /.test(line)) {
      ...
      elements.push(
        <ul key={kr.k++} className="my-1 list-disc pl-6">
          {items.map((it) => (
            <li key={kr.k++} className="text-[17px] leading-relaxed">
    ...
    elements.push(
      <p key={kr.k++} className="text-[17px] leading-relaxed">
